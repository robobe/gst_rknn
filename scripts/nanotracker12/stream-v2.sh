...
pipe="$env gst-launch-1.0 -q filesrc location='$root/assets/nanotracker/move_right_500.mp4' ! decodebin ! videoconvert ! video/x-raw,format=BGR,width=640,height=360,framerate=30/1 ! rknnnanotracker12 enabled=true roi='20,148,64,64' models-dir='$root/models/nanotracker12' model-version=v2 precision=mixed resize=auto ! roi2udp host='$host' port=$metadata_port ! videoconvert ! video/x-raw,format=I420 ! x264enc tune=zerolatency speed-preset=ultrafast bitrate=2000 key-int-max=30 bframes=0 ! h264parse ! rtph264pay pt=96 config-interval=1 ! udpsink host='$host' port=$video_port sync=false async=false"
