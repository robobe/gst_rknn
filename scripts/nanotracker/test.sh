...
usage() {
  printf '%s\n' \
    "usage: $0 {inspect|smoke|video|compare}" \
...
case "${1:-}" in
  inspect)
    ...
    ;;
  smoke)
    ssh "$target" "$environment gst-launch-1.0 -q videotestsrc num-buffers=60 ! videoconvert ! video/x-raw,format=BGR,width=320,height=240 ! rknnnanotrack enabled=true roi='80,60,64,64' models-dir='$remote_root/models/nanotracker' precision=mixed resize=auto ! fakesink"
    ;;
  video)
    pipeline="$environment gst-launch-1.0 -q filesrc location='$remote_root/assets/nanotracker/move_right_500.mp4' ! decodebin ! videoconvert ! video/x-raw,format=BGR,width=640,height=360 ! rknnnanotrack enabled=true roi='20,148,64,64' models-dir='$remote_root/models/nanotracker' precision=mixed resize=cpu ! roi2csv location='$remote_root/assets/nanotracker/predictions.csv' ! fakesink sync=false"
    printf 'Running on %s:\n%s\n' "$target" "$pipeline"
