...
def main():
    ...
    parser.add_argument("--profile", default="int8")
    ...
    detections = ground_truth.loadRes(str(args.dataset / f"predictions-{args.profile}.json"))
    evaluation = COCOeval(ground_truth, detections, "bbox"); evaluation.evaluate(); evaluation.accumulate(); evaluation.summarize()
    benchmark = json.loads((args.dataset / f"benchmark-{args.profile}.json").read_text())
    args.output.write_text(f"# YOLOv8n RKNN COCO val sample ({args.profile})\n\n"
                           f"- Date: {datetime.now(timezone.utc):%Y-%m-%dT%H:%M:%SZ}\n"
                           f"- Board: Radxa Zero 3W / RK3566\n"
                           f"- Runtime: RKNN 2.3.2\n"
                           f"- Model: `yolov8n-rk3566-{args.profile}.rknn`\n"
